...
window.initScroll=(options)=>{
	...
	if(options["pullDownLoading"]){
		var html="<div class='pull-down' style='position:absolute;top:-50px;height:50px;width:100%;line-height:50px;text-align:center;border:0;padding:0;margin:0;'>"
				+	"<img style='position:relative;height:30px;top:10px;margin-right:10px;-webkit-transition:-webkit-transform 0.1s ease-out;-webkit-transform:rotate(0deg);' src='"+require("./images/pull-down.png")+"'>"
	...
	if(options["pullUpLoading"]){
		var html="<div class='pull-up' style='position:absolute;bottom:-50px;height:50px;width:100%;line-height:50px;text-align:center;border:0;padding:0;margin:0;'>"
				+	"<img style='position:relative;height:30px;top:10px;margin-right:10px;-webkit-transition:-webkit-transform 0.1s ease-out;-webkit-transform:rotate(0deg);' src='"+require("./images/pull-up.png")+"'>"
    ...
    if( (dir=="x" && scrollDir=="x") || (dir=="y" && scrollDir=="y") ){
        ...
        if(touchEndOffset>0){
			...
			if(pullDownLoading && touchEndOffset>50){
				...
				$pullDown.find("img").attr("src",require("./images/loading.gif"));
				...
				pullDownLoading.call(window,function(listReset){
					...
					$pullDown.find("img")[0].style.webkitTransform="rotate(0deg)";
					$pullDown.find("img").attr("src",require("./images/pull-down.png"));
				})
			}else{
				currentTarget.style.webkitTransform=translate+"(0px)";
        ...
        if(touchEndOffset<domMoveAvl){
			...
			if(pullUpLoading && touchEndOffset<domMoveAvl-50){
				...
				$pullUp.find("span").text("正在加载，请稍候...");
				$pullUp.find("img")[0].style.webkitTransform="rotate(0deg)";
				$pullUp.find("img").attr("src",require("@/resource/images/loading.gif"));
...
(function(){
	window.showAlert=function(content,title,btn,btnFunction){
		...
		var html="<div class='max-dialog max-alert'>"
				...
				+			"<div class='max-dialog-text'>"+(content||"")+"</div>"
				+			"<div class='max-dialog-btn-group'>"
				+				"<span class='max-dialog-btn blue-button'>"+(btn||"确认")+"</span>"
				+			"</div>"
				+		"</div>"
				+	"</div>"
				+"</div>";*/
		
		var html = "<div class='jui_dialog_area'>"
				 + 	 "<div class='jui_dialog'>"
				 +	 	"<div class='jui_dialog_header'><strong>" + (title||"提示") + "</strong></div>"
				 +   	"<div class='jui_dialog_body'>" + (content||"提示") + "</div>"
				 +   	"<div class='jui_dialog_footer'>"
				 + 			"<a class='jui_dialog_btn_secondary'>取消</a>"
				 +			"<a class='jui_dialog_btn_primary'>"+(btn||"确认")+"</a>"
				 +	 	"</div>"
				 +	 "</div>"
				 + "</div>"
				 + "<div class='jui_mask'></div>";
		
		var $alert=$(html).appendTo(".ui-page-active");

		$alert.find(".jui_dialog_btn_primary").listen("quickClick",function(){
			if(btnFunction)
				btnFunction.call();
			$(".jui_dialog_area").remove();
			$(".jui_mask").remove();
		});
		
		$alert.find(".jui_dialog_btn_secondary").listen("quickClick",function(){
			$(".jui_dialog_area").remove();
			$(".jui_dialog_area").jui_mask();
		});
	}
	window.hideAlert=function(){
	    $(".jui_dialog_area").remove();
	...
	window.showConfirm=function(content,title,btn1,btnFunction1,btn2,btnFunction2){
		var html = "<div class='jui_dialog_area'>"
			 ...
			 +   	"<div class='jui_dialog_body'>" + (content||"提示") + "</div>"
			 +   	"<div class='jui_dialog_footer'>"
			 + 			"<a class='jui_dialog_btn_secondary'>" + (btn2||"取消") + "</a>"
			 +			"<a class='jui_dialog_btn_primary'>"+(btn1||"确认")+"</a>"
			 +	 	"</div>"
			 +	 "</div>"
			 + "</div>"
			 + "<div class='jui_mask'></div>";
		...
		$confirm.find(".jui_dialog_btn_primary").listen("quickClick",function(){
			if(btnFunction1)
				btnFunction1.call();
			$(".jui_dialog_area").remove();
			$(".jui_mask").remove();
		});
		$confirm.find(".jui_dialog_btn_secondary").listen("quickClick",function(){
			if(btnFunction2)
				btnFunction2.call();
			$(".jui_dialog_area").remove();
			$(".jui_mask").remove();
	...
	window.showLoading=function(text,autoHide,timer){
        // var html="<div class='max-dialog max-loading'>"
                // +   "<div class='max-dialog-masker'></div>"
                // +   "<div class='max-dialog-frame'>"
                // +       "<img src='"+window.resUrl+"images/loading.gif'>"
                // +       "<span>"+(text||"正在加载...")+"</span>"
                // +   "</div>"
                // +"</div>";
        var html = '<div class="jui_toast">'
				+	'<i class="jui_loading_01"></i>'
				+	'<p>'+(text||"正在加载...")+'</p>'
				+'</div>';
        var $confirm=$(html).appendTo(".ui-page-active");
	...
	// window.toast = function(text,timer){
	// 	 html = '<div class="jui_toast_text">'
	// 		  + 	'<p>' + text + '</p>'
	// 		  + '</div>';
	//
	//      var $confirm=$(html).appendTo(".ui-page-active");
     //     setTimeout(function(){
     //         $(".jui_toast_text").remove();
     //     },timer||2000);
	// }

    window.toast = function(text,timer,callback){
        var html = '<div class="jui_toast_text">'
            + 	'<p>' + text + '</p>'
            + '</div>';

        var $confirm=$(html).appendTo("body");
        setTimeout(function(){
            $(".jui_toast_text").remove();
            if(callback){
                callback();
