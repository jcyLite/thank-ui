...
function DynamicPic(options){
	...
	var dpHeight=options.height||(widnow.innerWidth*5/16);
	...
	if(options.thumbnail){
		...
		var thumbnailCount=options.thumbnailCount||2;
...
DynamicPic.prototype.playPic=function(event){
    ...
    if(event && event.data && event.data.direction){
		...
		if(this.options.autoSlide!=false)
			this.dpTimer=setInterval($.proxy(this.playPic,this),this.options.timer||5000);
