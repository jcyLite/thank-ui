...
//          console.info('cancel')
    ...
    function getActiveId($item,index){
        return $item.eq(index).find('li#active1').data('id')
    }
    ...
    function setActiveItem(obj, val) {
        ...
        //添加选中样式
//        obj.find('li').eq(index).addClass('active1').siblings('li').removeClass('active1');
        obj.find('li').eq(index).attr("id","active1").siblings('li').removeAttr("id");
        result = {
    ...
    function concatHtmlList(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            html += '<div class="mPicker-list"><ul data-role="listview">' + itemStr + '</ul></div>';
        }
        return html;
    }
    /**
