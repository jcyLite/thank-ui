<template>
	<!-- 忘记密码 -->
	<div v-if="show" class="revise-box" id="tip">
		<div class="revise-secondary">
			<div class="return" @layclick=close id="return">x</div>
			...
			<ul>
				...
				<li>
					<label for="identifying">验证码：
						<span :fgetc='fgetc' ref="fgetc" id="get-code" @layclick=get_code($event)>获取验证码</span>
...
<script>
	...
	export default{
		...
		computed:{
			show(){
				return this.$store.state.forget.show
			}						
		},
		created(){
		...
		methods:{
			...
			/* 模态框 */
			close() {
				this.$store.state.forget.show=!1;
			},
			/* 获取验证码 */
			get_code: function get_code(e) {				
				var _this = this;
				if(this.checkver_test()) {
					var phone = this.phone;
					if(phone == "") {
						this.phone_prompt = "手机号不能为空";
						return;
					}
					if(this.fgetc) {
						...
						//this.fgetc = false;
						//sadf.call(this)
						/*this.$http.post('?service=App.User.SmsSend',{'phone': phone}).then(d=>{							
							var i = 60;
			...
			check_register: function check_register(phone, code, password, username) {
				...
				this.$http.post('?service=App.User.ResetPasswd',
					{
						'phone': phone,
						"code": code,
						"password": password,
...
.revise-box .revise-secondary .return{
	float: right;
    display: block;
    width: 30px;
    height: 30px;
...
.revise-box .revise-secondary .title{
	text-align: center;
	font-size: 22px;
	color: #000000;
	margin-top: 30px;
}
.revise-box .revise-secondary >ul{
	width: 346px;
	margin: 30px auto;
}
.revise-box .revise-secondary >ul div{
	color:red;
}
.revise-box .revise-secondary >ul >li{
	width: 346px;
	height: 46px;
	margin: 11px 0;
}
.revise-box .revise-secondary >ul >li label{
	line-height:46px;
}
.revise-box .revise-secondary >ul >li label >input{
	width: 80%;
	height: 100%;
...
.revise-box .revise-secondary >ul >li label .right-input{
	float: right;
}
.revise-box .revise-secondary >ul >li label .identifying{
	width: 138px;
	float: right;
}
.revise-box .revise-secondary >ul >li label >span{
...
.revise-box .revise-secondary >ul .button{
	background-color: #11bdc5;
	color: #FFFFFF;
	font-size: 16px;
	text-align: center;
...
.revise-box  .get-code-true { 
	background-color: #11bdc5
}
.revise-box  .get-code-false {
