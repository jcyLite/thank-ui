...
$(function() {
    ...
    v.prototype = {
        ...
        render: function() {
            ...
            A = q.call(D, E);
            H = '<div class="mPicker-main ' + D.disy + '" data-pickerId="' + D.pickerId + '">' + D.options.header + '<div class="mPicker-content">' + A + '</div><div class="mPicker-shadow"></div>' + D.options.footer + "</div>";
            D.mpicker.append(H);
            D.mpickerMain = D.mpicker.find(".mPicker-main");
            var z = D.mpickerMain.find(".mPicker-content");
            var B = D.mpickerMain.find(".mPicker-list");
            var C = B.find("ul");
            D.options.level > 1 ? B.width((100 / D.options.level).toFixed(2) + "%") : false;
            B.append('<div class="mPicker-active-box"></div>');
            B.find(".mPicker-active-box").height(D.options.height);
            var G = D.options.rows % 2 === 0 ? -D.options.height - 2 + "px" : -D.options.height * 0.5 - 2 + "px";
            z.find(".mPicker-active-box").css({
                "margin-top": G
            });
        ...
        showPicker: function() {
            ...
            m(z.container, 0);
            x.each(function(B, C) {
                var A = z.container.data("id" + (B + 1)) ? z.container.data("id" + (B + 1)) : 0;
                y.push(A)
            });
            f.call(z, y)
        },
...
//			console.info(22, x.mask.hasClass("hide"));
//			console.info(22, x.mask);
            if(x.options.display === "bottom") {
                x.mpicker.find(".mPicker-main").addClass("down").transitionEnd(function() {
...
//			console.info("cancel");
        ...
        event: function() {
            var A = this;
            this.container.off("touchstart.container click.container").on("touchstart.container click.container", function(C) {
                C.preventDefault();
                C.stopPropagation();
                var B = $(this);
            ...
            this.mpicker.off("touchstart.confirm click.confirm").on("touchstart.confirm click.confirm", ".mPicker-confirm", function(B) {
                B.preventDefault();
                var C = $(".mPicker").data("object");
                C.confirm()
            });
            this.mpicker.off("touchstart.cancel click.cancel").on("touchstart.cancel click.cancel", ".mPicker-cancel", function(B) {
                B.preventDefault();
                var C = $(".mPicker").data("object");
                C.cancel()
            });
            this.mpicker.off("touchstart.mask click.mask").on("touchstart.mask click.mask", ".mPicker-mask", function(B) {
                B.preventDefault();
                var C = $(".mPicker").data("object");
                if(C.options.shadow) {
                    C.cancel()
            ...
            var x;
            var z;
            this.mpicker.off("touchstart.list mousedown.list").on("touchstart.list mousedown.list", ".mPicker-list", function(B) {
                g(B);
                var D = $(this).find("ul");
                var C = a(D);
                y = n(B).y - C;
            ...
            this.mpicker.off("touchmove.list mousemove.list").on("touchmove.list mousemove.list", ".mPicker-list", function(E) {
                E.preventDefault();
                if(!h) {
                    return false
                }
            ...
            this.mpicker.off("touchend.list mouseup.list").on("touchend.list mouseup.list", ".mPicker-list", function(B) {
                B.preventDefault();
    ...
    function l(x, y) {
        return x.eq(y).find("li#active1").data("id")
    }
    ...
    function p() {
        ...
        if(typeof(x) !== "object" || !x.length || !A.mpicker.find(".mPicker-main")) {
            return
        }
    ...
    function b(B, C) {
        ...
        var z = typeof(C) === "number" ? B.find("li").index(B.find('li[data-id="' + C + '"]')) : this.middleRowIndex - D;
        var A = -this.options.height * (z - this.middleRowIndex);
        m(B, A);
//		B.find("li").eq(z).addClass("active1").siblings("li").removeClass("active1");
        B.find("li").eq(z).attr("id","active1").siblings("li").removeAttr("id");
        x = {
            target: B.find("li").eq(z),
            index: z
        };
        return x
    }
    ...
    function q(A) {
        ...
        for(var y = 0; y < A.length; y++) {
            ...
            z += '<div class="mPicker-list"><ul data-role="listview">' + x + "</ul></div>"
        }
        return z
    }
